...
\begin{frame}
    ...
    % NP-hard!
    % Die optimale Lösung mit ILP-Solver in exponentieller Zeit
    % Heuristische/Approximative Lösungen in polynomieller Zeit
    % Partielle optimale Lösungen in polynomieller Zeit.
...
\begin{frame}
    ...
    % 2 alt: https://www.webofscience.com/wos/woscc/full-record/WOS:000683379203001
    \vspace{-5px}
    \textbf{Related Work:}\\
    {\footnotesize
    \begin{itemize}
        \item \cite{stein2023partial}
        \item \cite{lange2018partial}
        \item \cite{lange2019combinatorial}
    \end{itemize}
    }
    \onslide<2->{
    ...
    \begin{enumerate}
        \onslide<2->{
        \item Read \cite{stein2023partial}, implement the partial optimality algorithm\\
        $\to$ implementation in C++
        }
        \onslide<3->{
        \item Construct subspace instances of increasing difficulty\\
        $\to$ point generation, appropriate \textbf{cost function} $\cost$\\
        } 
        \onslide<4->{
